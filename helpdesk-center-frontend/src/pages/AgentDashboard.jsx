...
export default function AgentDashboard() {
  ...
  const listContent = (
    <div>
      ...
      {/* Archive read-only banner — persistent overlay (ui-ux-blueprint §3 Tab C) */}
      {activeTab === 'archive' && (
        <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-none">
          <span className="w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0" />
          <p className="text-xs font-semibold text-amber-700 dark:text-amber-400">
            Read-Only Mode: Teammate Assigned Workspace — Re-assign a ticket to yourself to edit it.
          </p>
        </div>
      )}
    </div>
